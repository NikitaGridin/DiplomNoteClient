...
const TrackForUser = observer(() => {
  ...
  const handleChangePart = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}libray/allTrack/${currentPart}/${userId}`);
  ...
  return (
    <div className='mb-[120px]'>
      ...
      <div className='grid grid-cols-2 gap-10 mb-14'>
        {tracks.map((track, i) => {
          return <Track key={i} num={i + 1} track={track} addedTracks={userStore.tracksData}  trackId={track.id} url={import.meta.env.VITE_AUDIO_URL+track.audio} img={import.meta.env.VITE_IMG_URL+track.Album.img} ttle={track.title} authorId={track.Album.User.id} authorNickname={track.Album.User.nickname} coautors={track.CoauthorAlias} delOn={deleteTrack}/>;
        })}
      </div>
      {
        tracks.length > 0 &&

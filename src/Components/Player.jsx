...
const Player = observer(() => {
  ...
  }, [currentTrackStore.currentTrack, currentTrackStore.isPlaying,currentTrackStore.currentTime]);
...
const handleRepeat = ()=>{
  ...
  return (
    <div className='fixed w-1/2 mx-auto left-0 right-0 bottom-5 flex bg-gray-100 shadow-xl rounded-xl py-2 px-7 items-center border'>
        ...
        <img className='w-10 h-10 object-cover rounded-2xl mr-5' src={currentTrack.img} alt="" />
        <div>
          ...
          <div>
            ...
            {currentTrack.coautors &&
                currentTrack.coautors.map((e,i)=> <span key={i} className='mr-2 text-xs font-normal text-gray-500'>{e.User.nickname}</span>)
            }
      ...
      <div className='mr-14 w-20 flex'>
        <img src={Volume} alt="" onClick={()=>{setModalVolume(!modalVolume)}} className='cursor-pointer mr-1'/>
        {
          modalVolume &&
            <input
              type="range"
      ...
      <div onClick={handleRepeat} className='cursor-pointer'>
        {currentTrackStore.repeat ? <img src={Repeat_a} alt="" /> : <img src={Repeat_d} alt="" />}
      </div>
    </div>
  );
});
